...
# def create_reservation(request):
#     """ Crée une nouvelle réservation. Vérifie que :
#     - le livre n'est pas déjà réservé
#     - le lecteur n'a pas déjà une réservation active
#     """
#     if request.method == 'POST':
#         form = ReservationForm(request.POST)
#         if form.is_valid():
#             reader = form.cleaned_data['reader']
#             # Vérifier si le lecteur a déjà une réservation active
#             if reader.get_active_reservations().exists():
#                 messages.error(request, f'Le lecteur "{reader.name}" a déjà une réservation en cours.')
#             else:
#                 try:
